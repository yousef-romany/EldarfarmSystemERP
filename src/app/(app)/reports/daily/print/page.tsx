...
const ReportContent = () => {
    ...
    if (isLoading) return <div className="p-8">جاري تحميل التقرير...</div>;
    if (error) return <div className="p-8">فشل في تحميل التقرير. يرجى التأكد من أنك متصل بالإنترنت وحاول مرة أخرى.</div>;
    if (!data || !data.wallets) return <div className="p-8">لا توجد بيانات لهذا اليوم.</div>;
  
    const { wallets, financialSummary, transactions } = data;
                    ...
                    <CardHeader className="p-0">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h1 className="text-2xl font-bold">التقرير المالي اليومي</h1>
                    ...
                    <CardContent className="p-0 mt-8">
                      <div className='mb-8 break-inside-avoid'>
                          <CardHeader className='p-0 mb-4'>
                              <CardTitle className='text-xl border-b pb-2 mb-2'>الحركات المالية لليوم</CardTitle>
                          </CardHeader>
                          <div className="grid grid-cols-2 gap-8">
                              <div>
                                  <h4 className="font-bold mb-2 text-green-600">المقبوضات</h4>
                                  <Table>
                                      <TableHeader><TableRow><TableHead>البيان</TableHead><TableHead className='text-right'>المبلغ</TableHead></TableRow></TableHeader>
                                      <TableBody>
                                          {transactions.inflows.map((inflow, i) => (
                                          ...
                                          {transactions.inflows.length === 0 && <TableRow><TableCell colSpan={2} className='text-center text-muted-foreground'>لا توجد مقبوضات</TableCell></TableRow>}
                                      </TableBody>
                                  </Table>
                              </div>
                               <div>
                                  <h4 className="font-bold mb-2 text-red-600">المدفوعات</h4>
                                   <Table>
                                      <TableHeader><TableRow><TableHead>البيان</TableHead><TableHead className='text-right'>المبلغ</TableHead></TableRow></TableHeader>
                                      <TableBody>
                                           {transactions.outflows.map((outflow, i) => (
                                           ...
                                           {transactions.outflows.length === 0 && <TableRow><TableCell colSpan={2} className='text-center text-muted-foreground'>لا توجد مدفوعات</TableCell></TableRow>}
                                      </TableBody>
                                  </Table>
                              </div>
                          </div>
                          <CardFooter className='p-0 mt-4 flex flex-col items-end space-y-2'>
                              ...
                              <div className="flex justify-between w-full font-semibold">
                                  ...
                                  <span className="text-green-600">{formatCurrency(financialSummary.totalIn)}</span>
                              </div>
                              <div className="flex justify-between w-full font-semibold">
                                  <span>إجمالي المدفوعات:</span>
                                  <span className="text-red-600">{formatCurrency(financialSummary.totalOut)}</span>
                              </div>
                               <Separator className="my-2" />
                               <div className="flex justify-between w-full font-bold text-lg">
                                  <span>صافي الحركة لليوم:</span>
                                  <span>{formatCurrency(financialSummary.netChange)}</span>
                              </div>
                          </CardFooter>
                        </div>
                      <div className='break-before-page'></div>
                      <CardHeader className='p-0 mb-4 mt-8'>
                          <CardTitle className='text-xl border-b pb-2 mb-2'>ملخص الأرصدة النهائية للمحافظ</CardTitle>
                      </CardHeader>
                      <Table>
                      ...
                      <CardFooter className='p-0 mt-8 flex flex-col items-end space-y-2 bg-muted p-4 rounded-lg'>
                         ...
                         <div className="flex justify-between w-full font-bold text-2xl">
                            <span>إجمالي الأرصدة الحالية:</span>
