...
export default function DeferredSaleClientPage({ availableLivestock, wallets }: { availableLivestock: Livestock[], wallets: Wallet[]}) {
  ...
  return (
    <>
      <PageHeader title="تسجيل بيع آجل" />
       <form action={createFormAction}>
        <Card className="max-w-4xl mx-auto">
            <CardHeader>
                <CardTitle>تفاصيل عملية البيع الآجل</CardTitle>
                <CardDescription>
                    املأ البيانات أدناه لتسجيل حجز لحيوان سيتم تسليمه وتسويته لاحقًا. يجب دفع عربون.
                </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                <input type="hidden" name="saleType" value="Deferred" />
                ...
                <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div className="grid gap-2">
                        ...
                        <Input name="customerName" id="customer-name" placeholder="اسم المشتري" value={customerName} onChange={e => setCustomerName(e.target.value)} required />
                    </div>
                </div>
            ...
            <CardHeader>
                <CardTitle>دفع العربون</CardTitle>
            </CardHeader>
             <CardContent className="space-y-4">
             ...
             <CardContent>
                <div className='flex justify-between items-center p-3 bg-muted rounded-md mb-2'>
                    <span className='font-semibold'>الإجمالي المدفوع (العربون):</span>
                ...
                <div className='flex justify-between items-center p-3 bg-muted rounded-md'>
                    <span className='font-semibold'>المبلغ المتبقي (مبدئيًا):</span>
                    <span className='font-bold text-lg'>{new Intl.NumberFormat('ar-EG', { style: 'currency', currency: 'EGP' }).format(remainingBalance)}</span>
                </div>
            </CardContent>
            <CardContent>
                 <SubmitButton text="تسجيل البيع الآجل" disabled={!selectedAnimal || !pricePerKg || totalPaid === 0} />
            </CardContent>
        </Card>
        </form>
    </>
